"""This script demonstrates how you cannot query for screens directly using `query_one` or `query_all`.
Instead, you should use the `get_screen` method to retrieve the screen instance.
Attempting to query for the screen as shown below will cause the app to crash.
Also note that the `get_default_screen` method is not needed
if you are using `push_screen` to intialize with an installed screen (in SCREENS).
This can cause issues because the default MainScreen instance will not be the same as the
